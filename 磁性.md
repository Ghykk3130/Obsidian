...
$$\langle B\rangle=\langle {B}_{i}\rangle= \frac{1}{\mu}J \langle \sigma\rangle+B $$
则现在问题和无耦合自旋系统一样。于是显然：
$$M=n\mu \tanh(\beta \mu \langle {B}\rangle)$$
我们可以求解上面假设的平均值
另一方面，我们又知道：
$$M=n\mu \langle \sigma\rangle=n\mu  \langle {\sigma}\rangle$$
于是必有：
$$\langle \sigma\rangle=\tanh(\beta \mu \langle {B}\rangle)$$
于是得到方程：
$$\langle \sigma \rangle=\tanh(\beta J \langle \sigma \rangle+\beta \mu B)$$

如果要考虑自发磁化，则令外场$B=0$。则需要解：
$$\langle \sigma \rangle=\tanh(\beta J\langle \sigma\rangle)$$
