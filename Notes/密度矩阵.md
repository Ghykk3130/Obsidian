...
令$\rho=\sum_{j}p_{j}\ket{\psi_{j}}\bra{\psi_{j}}$。那么：
$$\begin{align}
Tr(\rho) & = \sum_{j,n}p_{j}|\bra{n} \psi_{j}\rangle |^{2} \\
 & = \sum_{j}p_{j}=1
\end{align}$$
>[!Right]
>$\blacksquare$
...
结合上面两个proposition，一个二维Hilbert空间中密度矩阵的general形式为：
$$(\rho)=\begin{pmatrix}
a & b \\
b^{*} & 1-a
\end{pmatrix},\ a\in [0,1],b\in \mathbb{C}$$

>[!Success] Proposition 4.3
>The ensemble is pure if and only if $\rho^{2}=\rho$.
## Proof.

($\implies$:)显然。
($\impliedby$:) 因为$\rho$ Hermitian，不妨选择一组正交基使得$(\rho)$对角化。那么：
$$\rho^{2}=\rho \implies \rho_{jj}^{2}=\rho_{jj}\implies \rho_{jj}=0\text{ or }1,\ \forall j$$
但是$Tr(\rho)=1$，所以只能选择一个$\rho_{jj}=1$，其余为零。那么$\rho=\ket{j}\bra{j}$，ensemble is pure.
>[!Right]
>$\blacksquare$

一个等价命题为：

>[!Success] Proposition 4.4
>The ensemble is pure if and only if $Tr(\rho^{2})=1$.
## Proof.

($\implies$:)显然。
($\impliedby$:) 因为$\rho$ Hermitian，不妨选择一组正交基使得$(\rho)$对角化。那么：
$$Tr(\rho^{2})=1\implies \sum_{j}\rho_{jj}^{2}=1,\ Tr(\rho)=1\implies \sum_{j}\rho_{jj}=1\tag{*}$$
容易证明$\rho=\sum_{i}p_{i}\ket{\psi_{i}}\bra{\psi_{i}}\implies \rho_{jj}=\bra{j}\rho \ket{j}=\sum_{i}p_{i}|\bra{j}\psi_{i}\rangle|^{2}\in \mathbb{R}$。故$(*)$只有当某个$\rho_{jj}=1$，其余为零时才能满足。
>[!Right]
>$\blacksquare$
# 5. 系综的量子化
...
根据[[密度矩阵#^7b6ad6|definition 3.1]]，我们必须认为，这个状态对应的密度算子不随时间变动。则$\frac{d\rho}{dt}=0$。那么由von Neumann's equation，我们有：
$$[H,\rho]=0$$
...
## Proof.
由[[密度矩阵#^a34adb|proposition 4.2.1]]就是显然。
>[!Right]
>$\blacksquare$
