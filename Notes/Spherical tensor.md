...
考虑一个Cartesian并矢$U_{i}V_{j}$。我们将其耦合成球张量。$T^{(k)}_{q}$。首先必有：
$$\begin{align}
T^{(2)}_{\pm 2}= U^{(1)}_{\pm 1 }V^{(1)}_{\pm 1}
\end{align}$$
然后我们有：
$$\begin{align}
 & J_{-}T^{(2) }_{ 2}= J_{-}U_{ 1}^{(1)}V^{(1)}_{1} \\
\implies & \hbar { 2 }T^{(2)}_{1}=(J_{-}U_{1}^{(1)})V_{1}^{(1)}+U_{1}^{(1)}(J_{-}V^{(1)}_{1}) \\
\implies & \hbar { 2 }T^{(2)}_{1}=  \hbar \sqrt{ 2 }U_{0}^{(1)}V_{1}^{(1)}+ \hbar \sqrt{ 2 }U_{1}^{(1)}V^{(1)}_{0} \\
\implies & T^{(2)}_{1}= \frac{1}{\sqrt{ 2 }}(U^{(1)}_{0}V^{(1)}_{1}+ U^{(1)}_{1}V^{(1)}_{0})
\end{align}$$
类似地，我们可以得到所有的$T^{(2)}$。

类似地，通过CG系数表，我们有：
$$T^{(1)}_{ 1}= \frac{1}{\sqrt{ 2 }}(U^{(1)}_{1}V^{(1)}_{0}-U^{(1)}_{0}V^{(1)}_{1})$$
类似地，可以通过查CG系数表把所有的耦合都找出来。理论上来说，把所有的$U^{(1)},V^{(1)}$换成Cartesian坐标，再反解，就可以获得$U_{i}V_{j}$的求张量分解。
