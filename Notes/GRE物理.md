...
\end{align}$$
6. 金属体系载流子浓度随温度近似不变。电导率随温度的改变主要由电子-声子散射引起。半导体体系载流子浓度随温度强烈变化。电导率随温度的额改变主要由热涨落引起。
7. 主量子数的字母，K,L,M,N分别对应$n=1,2,3,4$。
8. 实际上，对于核电荷为$Z|e|$的类氢原子，相应电离能应该是$E\propto Z^{2}$。不能naive地认为，只是库伦作用增加了$Z$倍。实际上，电子还拉近了，与核的距离为原来的$\frac{1}{Z}$。
9. ![[Pasted image 20260731170126.png|center|300]]
...
10. Compton散射公式：$\Delta \lambda= \frac{h}{Mc}(1-\cos \theta)$。其中，$\Delta \lambda=\lambda^{'}-\lambda$为散射前后光子波长变化。$M$为被光子碰撞的粒子的静质量。$\theta$为散射角。
...
我们考虑的矩阵元是$\bra{n^{'},l^{'},m_{l}^{'},s ^{'}, m_{s}^{'}}\mathbf{r}\ket{n,l,m_{l},s,m_{s}}$。我们量子化的轴是z轴，但是电场不一定是沿着z轴的，所以必须要考虑$\mathbf{r}$的矩阵元，而不是只是$z$的矩阵元。我们知道$\mathbf{r}$可以分解为球张量$r^{(1)}_{0}= z,\ r^{(1)}_{\pm_{1}}= \mp\frac{x\pm iy}{\sqrt{ 2 }}$，然后用Wigner-Eckart定理得到selection rule。但是，由于$r^{(1)}$算子只和轨道角动量作用，所以只需要对于$l,m_{l}$用selection rule即可。选$。
...
那么对于眼睛来说，最终像的张角近似为$-w^{'}$。而显然，真实天体的张角近似为$w$。那么显然角放大率为$M=\frac{-w^{'}}{w}= -\frac{f_{o}}{f_{e}}$。直觉上来讲，$f_{o}$越大，那么$y$就越大。所以$M\propto f_{o}$。而$f_{e}$越小，偏转能力越强。所以放地越大。所以$M\propto \frac{1}{f_{e}}$。
...
转动惯量是$m\left(  \frac{b}{2} \right)^{2}+ m\left(  \frac{b}{2} \right)^{2}$！两个要加起来！选C。
...
47. ![[Pasted image 20260811152731.png|centering|200]]如果电容和电感的impedance没有完全抵消，那么总impedance总比$R$大一点点，导致电流不够小。以impedance的那个复平面图来思考。选C。
...
只需要记住，质子质量大约为电子质量2000倍。转动能的估计为$\frac{\hbar^{2}}{2\mu}L(L+1)$。其中$\mu \approx m_{H}r^{2}$，$r\sim 1\ \mathring{A}$。选B。
...
注意，这里给的是频率，不是角频率。应该对cyclytron frequency这个词抱有警惕。
...
碰撞前四动量为$(m+p,\mathbf{p})$。碰撞后四动量为$\left( 3\sqrt{ m^{2}+ \frac{p^{2}}{9} }, \mathbf{p} \right)$。四动量守恒得到$p=4m$。选D。
...
81. 对于光子气体，$U=aVT^{4}$。这是显然，因为黑体辐射$\propto T^{4}$，正是光子气体的能流密度。容易计算$p= \frac{1}{3}aT^{4}$。
![[Pasted image 20260814150751.png|centering|300]]
将宇宙膨胀当作光子气体的绝热膨胀。那么$dU=-pdV$。将所有变量代换为$V,T$，可以解关于$V,T$的微分方程。得到$TV^{1/3}=\text{const.}$。于是温度升高4倍，体积就是原来的$\frac{1}{64}$，长度就是原来的$\frac{1}{4}$。当然更简洁的做法是使用Wien位移定律。因为宇宙膨胀，所以辐射强度最大的光子波长变为原先$a$倍。这导致温度的改变。由$aT=\text{const.}$得到$1 /4$。

82. 绝热膨胀$TV^{\gamma-1}=\text{const.}$。等价于$pV^{\gamma}=\text{const.}$。对于理想单原子气体，$\gamma= \frac{5}{3}$。
83. 
![[Pasted image 20260814153852.png|300]]
这题给了$\theta$什么的，让人想要动量守恒来看。但这是恶意引导。只需要能量守恒来算就可以了。选C。
...
碰撞前四动量为$(M,0)$。碰撞后四动量为$(\sqrt{ m^{2}+p^{2} }+p,0)$。四动量守恒得到B。
...
94. ![[Pasted image 20260824161231.png|centering|300]]
形成n-type半导体，只需要dopant的价电子更多就可以了。背一下元素周期表。选D。
95. ![[Pasted image 20260824163107.png|centering|300]]
质量轻的粒子不能衰变为质量重的粒子。常见粒子质量大小为$\tau > p \approx n > \pi^{\pm}\approx \pi^{0}>\mu^{\pm}>e^{\pm}$。轻子有三种$\tau,\ \mu,\ e$，以及它们相应的中微子。这三种粒子每个具有自己的轻子数。例如$L_{e^{-}}=+1,\ \nu_{e}=+1, L_{\mu^{-}}=+1,\ \nu_{\mu}=+1,\ L_{\tau^{-}}=+1,\ \nu_{\tau}=+1$。它们的反粒子轻子数相反。这三种轻子各自的轻子数必须守恒。

来看D。mu子轻子数反应前$-1$，反应后$-1$，守恒。电子轻子数反应前$0$，反应后$-1+1=0$。守恒。选D。  

96. ![[Pasted image 20260906164825.png|centering|400]]
看清楚，问的是波长，不是频率。选A。
97. ![[Pasted image 20260906165042.png|centering|400]]
