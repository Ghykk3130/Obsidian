...
($\implies$：)显然。
($\impliedby$：) FTOSC，假设ensemble不pure。令$\rho=\sum_{j}p_{j}\ket{\psi_{j}}\bra{\psi_{j}}$。那么：
$$\rho^{2}=\sum_{i,j}p_{i}p_{j}\ket{\psi_{j}} \bra{\psi_{j}} \psi_{i}\rangle\bra{\psi_{i}}  $$
$$\begin{align}
Tr(\rho^{2})= \sum_{i,j,n}p_{i}p_{j}
\end{align}$$
