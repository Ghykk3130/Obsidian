...
考虑边界条件：
$$\partial_{z}\phi=0\text{ on the seabed}\implies \partial_{z}\tilde{\phi}=0\text{ at }z=-H$$
重写系数得到：
...
\end{align}$$
那么：
$$\partial_{z}\tilde{\phi}|_{z=0}= \frac{\omega^{2}}{g}\tilde{\phi}|_{z=0}\implies \omega^{2}=k_{z}g \tanh(k_{z}H)$$
>[!Success] Proposition 3.1
>The dispersion relation is:
>$$\omega^{2}=kg\tanh(kH)\text{, where }k=\sqrt{ k_{x}^{2}+k_{z}^{2} }$$
