...
\end{align}$$
假设$\partial_{x}h,\ \partial_{y}h\ll 1$，$\partial_{x}\phi,\ \partial_{y}\phi,\ \partial^{2}_{z}\phi \ll 1$，它们的任意阶导也为小量。我们忽略二阶小量，得到：
$$\begin{align}
 &  \frac{1}{2}\rho|\nabla \phi |^{2}+\rho \frac{\partial \phi}{\partial t}+p_{0}+\rho gh=f(t)\implies \frac{\partial \phi}{\partial t}+gh\approx f^{'}(t)\text{ on the boundary, where }f^{'}(t)= \frac{f(t)-p_{0}}{\rho}  \end{align}
 $$
我们将$f^{'}$写作$f$，得到：
$$\partial_{t}\phi+gh= f(t)\text{ on the surface}$$
另外：
...
>$$\begin{align}
 & \frac{\partial \phi}{\partial t}+gh= f^{}(t)\text{ on the surface} \\
...
$$\phi(x,y,z,t)= \int_{\mathbb{R}^{3}}dk_{x}dk_{y}d\omega \tilde{\phi}(k_{x},k_{y},z,\omega)e^{i(k_{x}x+k_{y}y-\omega t)}$$
那么：
$$\nabla^{2}\phi=0\implies \partial_{z}^{2}\tilde{\phi}=(k_{x}^{2}+k_{y}^{2})\tilde{\phi}$$
容易解得：
$$\tilde{\phi}=Ae^{k_{z}z}+B e^{-k_{z}z}\text{ where }k_{z}=\sqrt{ k_{x}^{2}+k_{y}^{2} }$$
考虑边界条件：
$$\partial_{z}\phi=0\text{ on the seabed}\implies \partial_{z}\tilde{\phi}=0\text{ on the seabed}$$
重写系数得到：
$$\tilde{\phi}=A \cosh(k_{z}(z+H))$$
考虑其余两个边界条件：
$$\begin{align}
 & \partial_{z}\phi=\partial_{t}h\text{ at }z=0\implies \partial_{z}\tilde{\phi}=-i\omega \tilde{h}\text{ at }z=0 \\
 & \partial_{t}\phi+gh=f^{}(t)\text{ on the surface}\implies -i\omega \tilde{\phi}+g\tilde{h}= \tilde{f^{}}\text{ on the surface}
\end{align}$$
由于$\tilde{f}$不是$z$的函数，那么：
$$\begin{align}
 & \partial_{z}\left( \tilde{\phi}+ \frac{\tilde{f}}{i\omega} \right)=-i\omega \tilde{h} \\
 & \tilde{\phi}+ \frac{\tilde{f}}{i\omega}= \frac{g}{i\omega}\tilde{h}
\end{align}$$
容易解得：
$$\tilde{h}= B\exp\left( \frac{\omega^{2}}{g}z \right)$$
