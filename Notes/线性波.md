...
不会影响物理速度$\mathbf{u}=\nabla \phi$。我们选取规范以至于：
$$\frac{\partial \phi}{\partial t}+gh=0\text{ on the surface}$$
其余两个边界条件显然不受影响。
...
$$\begin{align}
 ...
 & \partial_{t}\phi+gh=0\text{ on the surface}\implies -i\omega \tilde{\phi}+g\tilde{h}= 0\text{ on the surface}
\end{align}$$
由于$\tilde{f}$不是$z$的函数，那么：
$$\begin{align}
 & \partial_{z}\tilde{\phi}=-i\omega \tilde{h} \text{ at }z=0\\
 & \tilde{\phi}= \frac{g}{i\omega}\tilde{h}\text{ on the surface}
\end{align}$$
