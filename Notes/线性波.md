...
\end{align}$$
我们提出Fourier试探解：
$$\phi=A(y) \exp(i(kx-\omega t))$$
>[!Quote] Idea 4.1
>Fourier试探解其实就是Fourier变换假设了$\tilde{\phi}(k^{'},\omega^{'},y)=\delta(k^{'}-k)\delta(\omega^{'}-\omega)A(y)$，只有一个频率的峰。

那么：
$$\begin{align}
 & \nabla^{2}\phi=0\implies \partial_{yy}A=k^{2}A\implies A(y)=Be^{ky}+Ce^{-ky}
\end{align}$$
然后：
$$\partial_{y}\phi=0\text{ at }y=-H\implies A(y)=B\cosh(k(y+H))$$
于是：
$$\begin{align}
\left.\begin{aligned}
 & \partial_{y}\phi=\partial_{t}h\text{ at y=0} \\
...
\end{align}$$
得到色散关系。容易解得速度势：
$$\phi=B\cosh(k(y+H))e^{i(kx-\omega t)}$$
求梯度取实部得到速度场：
$$\begin{align}
\mathbf{u}=Bk\begin{pmatrix}-
...
\end{pmatrix}$$
积分得到pathline：
$$\mathbf{r}=- \frac{Bk}{\omega}\begin{pmatrix}
\cosh(k(y_{0}+h))\cos(kx_{0}-\omega t) \\
\sinh(k(y_0+h))\sin(kx_{0}-\omega t)
\end{pmatrix}+\mathbf{r}_{0}$$
这显然是一个椭圆：
$$\frac{(x-x_{0})^{2}}{\left[ \frac{Bk}{\omega} {\cosh(k(y_{0}+h))} \right]^{2}}+ \frac{(y-y_{0})^{2}}{\left[ \frac{Bk}{\omega} {\sinh(k(y_{0}+h))} \right]^{2}}=1$$
越靠近海底，椭圆长短轴都越小，几乎是指数衰减，并且椭圆越扁平。$h$越大，椭圆长短轴越大。若取$h\rightarrow \infty$，显然长轴短轴相同，得到圆形pathline。

The representative pathlines are drawn for $k=1,h=1,B=0.5,g=9.8$
