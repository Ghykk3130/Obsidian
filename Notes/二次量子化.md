...
我们可以证明原子完全填充的层总角动量为零。假设原子内电子弱相互作用。 令$a_{n,l,j,m_{j}}$为$\ket{n,l,j,m_{j}}$对应湮灭算符。那么某个$n,l$全部填满的态为：
$$\ket{\psi} = \prod_{j,m_{j}}a^{\dagger}_{n,l,j,m_{j}}\ket{0} $$
我们考虑单个粒子$\mathbf{J}$算符：
$$\begin{align}
J_{z} & = \sum_{n,l,j,m_{j},n^{'},l^{'},j^{'},m_{j}^{'}}\bra{n,l,j,m_{j}} J_{z}\ket{n^{'},l^{'},j^{'},m_{j}^{'}} a_{n,l,j,m_{j}}^{\dagger}a_{n^{'},l^{'},j^{'},m_{j}^{'}} \\
 & = \sum_{n,l,j,m_{j}} \hbar m_{j}a^{\dagger}_{n,l,j,m_{j}}a_{n,l,j,m_{j}} \\
 & =\sum_{n,l,j,m_{j}}\hbar m_{j}N_{n,l,j,m_{j}} \\
J_{\pm} & = \sum_{n,l,j,m_{j},n^{'},l^{'},j^{'},m_{j}^{'}}\bra{n,l,j,m_{j}} J_{\pm}\ket{n^{'},l^{'},j^{'},m_{j}^{'}} a_{n,l,j,m_{j}}^{\dagger}a_{n^{'},l^{'},j^{'},m_{j}^{'}} \\
 & = \sum_{n,l,j,m_{j}} \hbar \sqrt{ j(j+1)- m_{j}(m_{j}\mp 1) } a_{n,l,j,m_{j}}^{\dagger}a_{n,l,j,m_{j}\mp1} \end{align}$$
 那么多体算符只是单体算符张量积。故：
 $$\begin{align}
J_{z} & = 
\end{align}$$
