...
考虑一个量子多体系统。令$k_{i}$表示第$i$个量子态。则系统的态为$\ket{k_{i_{1}},k_{i_{2}},\dots}$。我们令$n_{i}:=\text{the number of particles in state }k_{i}$。我们试图用占据数的notation $\ket{n_{1},n_{2},\dots}$来表示$\ket{k_{i_{1}},k_{i_{2}},\dots}$。显然without ambiguity地：
$$
...
>Let $\ket{\{ n_{j} \}},\ \ket{\{ m_{j} \}}$ be Fock states. Then:
>$$\bra{\{ n_{j} \}} \{ m_{j} \}\rangle= \delta_{n_{1},m_{1}}\delta_{n_{2},m_{2}}\dots$$
## Proof.

若$\{ n_{j} \}=\{ m_{j} \}$，那么必有$\bra{\{ n_{j} \}}\{ m_{j} \}\rangle= 1$。若$n_{k}\neq m_{k}\text{ for some }k$，那么：
$$\begin{align}
 \bra{\{ n_{j} \}} N_{k}\ket{\{ m_{j} \}}  & = n_{k}\bra{\{ n_{j} \}} \{ m_{j} \}\rangle
\end{align}$$
另一方面：
$$\bra{\{ n_{j} \}} N_{k}\ket{\{ m_{j} \}} =m_{k}\bra{\{ n_{j} \}} \{ m_{j} \}\rangle$$
所以必有：
$$\bra{\{ n_{j} \}} \{ m_{j} \}\rangle =0 $$
>[!Right]
>$\blacksquare$
>
...
我们可以证明原子完全填充的层总角动量为零。假设原子内电子弱相互作用。 令Fock state为$\ket{n,l,m_{l}}$，$\ket{n,l,m_{s}}$所组成。固定某个$n,l$全部填满的态为：
$$\ket{\psi} = \prod_{m_{l},m_{s}}a_{m_{l}}^{\dagger}a_{m_{s}}^{\dagger}\ket{0} $$
由[[二次量子化#^afb5ae|proposition 2.1]], [[二次量子化#^bee825|proposition 2.2]]，我们考虑additive 算符$\mathbf{L}$：
$$\begin{align}
L_{z} & = \sum_{m_{l}} m_{l}\hbar a_{m_{l}}^{\dagger}a_{m_{l}} \\

L_{\pm} & = \sum_{m_{l},m_{l}^{'} } a_{m_{l}}^{\dagger}a_{m_{l}^{'}}^{} \bra{m_{l}} L_{\pm} \ket{m_{l}^{'}} = \sum_{m_{l}}\hbar \sqrt{ l(l+1)-m_{l}(m_{l} \mp  1) } a_{m_{l}}^{\dagger}a_{m_{l} \mp 1}\end{align}$$
同理：
$$\begin{align}
 & S_{z}=\sum_{m_{s}}m_{s}\hbar a_{m_{s}}^{\dagger} a_{m_{s}} \\
 & S_{\pm}= \sum_{m_{s}}\hbar \sqrt{ \frac{3}{4}- m_{s}(m_{s} \mp 1) } a_{m_{s}}^{\dagger} a_{m_{s} \mp 1}
\end{align}$$
显然：
$$\begin{align}
L_{z} \ket{\psi}  & = \sum_{m_{l}=-l }^{l} m_{l}\hbar N_{m_{l}} \ket{\psi}  \\
 & = \sum_{m_{l}} m_{l}\hbar \ket{\psi} \\
 & = 0
\end{align}$$
这是因为$\ket{m_{l}}$必须完全占满，所以$N_{m_{l}}\ket{\psi}=1,\ \forall m_{l}$。同理$S_{z}\ket{\psi}=0$。接下来：
$$a_{m_{l}}^{\dagger} a_{m_{l}\mp 1}\ket{\psi} =0$$
这是因为$\ket{m_{l}}$已经占满，而再用$a_{m_{l}}^{\dagger}$作用只能得到零。所以$L_{\pm}\ket{\psi}=0$。同理$S_{\pm}\ket{\psi}=0$。所以$\mathbf{J}\ket{\psi}=0$。
