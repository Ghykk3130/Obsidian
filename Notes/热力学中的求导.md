...
\end{align} \dots *)$$
>[!Warning]
>对于热容，定义始终是$C_{x}= T\left(  \frac{\partial S}{\partial T} \right)_{x}$，而不是$\left( \frac{\partial U}{\partial T} \right)_{x}$。热容始终表示吸热。而吸热始终为$TdS$。
...
>[!Quote] $C_{p}>C_{V}$的直觉
>维持$p$不变，系统升温时必须膨胀。那么会对外做功。也就是说，系统吸收的热量$TdS$分为两部分。系统先吸收了热量，增加了内能。但是由于膨胀，一部分能量被用作向外做功了，所以系统可以“吸更多热”。
