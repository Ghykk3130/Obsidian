...
我们称$\Gamma$为环量，并注意到：
$$u_{r}= U- \frac{r_{0}^{2}}{r^{2}}U,\ u_{\theta}= \frac{\Gamma}{2\pi r}$$
那么：
$$\mathbf{u}\cdot d  \mathbf{r}= u_{r}dr+ u_{\theta }r  d\theta$$
那么显然沿着包围原点的曲线$C$积分，我们有：
$$\begin{align}
\oint_{C}\mathbf{u}\cdot d\mathbf{r} & = \oint u_{r}dr+ \oint u_{\theta}r d\theta \\
 & = \oint u_{\theta}r d\theta \\
 & = \Gamma
\end{align}$$
我们可以进一步求出复速度势。我们有：
$$\begin{align}
