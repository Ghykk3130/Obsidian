...
>[!Note] Definition 3.1
>Operator $A$ is called a complex conjugate operator if given some basis $\ket{n}$, we have:
>$$A\left( \sum_{n}c_{n}\ket{n}  \right)=\sum_{n}c_{n}^{*}\ket{n} $$
>Operator $A$ is antilinear if:
>$$A(a\ket{\psi} +\ket{\phi} )=a^{*}A\ket{\psi} +A\ket{\phi} ,\ \forall a,\ \ket{\psi} ,\ket{\phi} $$
>Operator $A$ is anti-unitary if it is antilinear, and:
>$$\bra{\psi} \phi\rangle= \bra{\tilde{\psi}} \tilde{\phi}\rangle^{*}\text{ where }\ket{\tilde{\psi}} =A\ket{\psi} ,\ \ket{\tilde{\phi}} =A\ket{\phi} $$
## Remark
...
$$A\ket{n} =1^{*}\ket{n} =\ket{n} $$

>[!Success] Proposition 3.1
>Given $A$ an anti-unitary operator, there exists a complex conjugate operator $K$ and a unitary operator $U$ such that:
>$$A=UK$$
## Proof.
...
于是得证。那么令$U=AK^{-1}=AK$。想要证$U$是酉算子。首先证明$U$是线性的：
$$\begin{align}
 U(a\ket{n_{1}} +\ket{n_{2}} ) &=AK(a\ket{n_{1}} +\ket{n_{2}} ) \\
 & = A(a^{*}\ket{n_{1}} +\ket{n_{2}} ) \\
 & = aA\ket{n_{1}} +A\ket{n_{2}}  \\
 & = aAK\ket{n_{1}} +AK\ket{n_{2}}  \\
 & = aU\ket{n_{1}} +U\ket{n_{2}}  
\end{align}$$
接下来证明$U$是酉的。只需要对于基证明即可。我们作：
$$\begin{align}
\bra{U n_{1}} Un_{2}\rangle & = \bra{AKn_{1}} AKn_{2}\rangle \\
