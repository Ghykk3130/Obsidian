...
Unit cell指只要通过平移能够tile整个平面的cell。可以是六边形什么的。而primitive cell要求平移的一定是格矢。所以primitive是一种特殊的unit cell。primitive cell会将所有真实的原子当作格点。所以一个primitive cell只会包含一个原子。
...
给定$\Lambda ^{*}$，我们定义Brillouin区：

>[!Note] Definition 4.1
>Define the nth Brillouin zone as a subset $\text{BZ}_{n} \subset \mathbb{R}^{d}$, such that $0$ is the nth nearest point in $\Lambda ^{*}$ for points in $\text{BZ}_{n}$.

我们再定义Bragg面为$\mathbf{G}$与原点的平分面。先对于所有$\mathbf{G}\in \Lambda,\ \mathbf{G}\neq 0$作Bragg面。想知道$\mathbf{k}$属于第几个BZ，只需要从原点开始，走直线到$\mathbf{k}$。若穿过n次Bragg面，则$\mathbf{k}\in\text{BZ}_{n+1}$。

>[!Quote] 如何理解这个判断方法
>我们知道，在$\mathbf{G}$与原点构造的Bragg面靠原点的一侧，则距离原点比距离$\mathbf{G}$点更近。我们从原点开始走，最开始距离原点非常近，所以原点是最近的点。往外走若穿过一个Bragg面，则代表距离原点第二近，距离这个Bragg面对应的$\mathbf{G}$点最近。此时因为没有穿过其它的Bragg面，所以距离原点比距离任意其他$\mathbf{G}^{'}\neq \mathbf{G}$更近。那么往后每穿过Bragg面一次，原点的排序就往后退一位。并且只会退一位，因为一个Bragg面对应一个倒格点。

## Ex:

我们以二维suqare lattice为例。FBZ：
![[Drawing 2026-04-14 18.58.39.excalidraw|center|500]]
$\text{BZ}_{2}$：
![[Drawing 2026-04-14 19.11.17.excalidraw|center]]
接下来的构造以此类推。
