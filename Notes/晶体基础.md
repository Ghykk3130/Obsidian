...
我们以二维suqare lattice为例。

为了构造FBZ，我们选择与原点最相近的点，将它们构成的集合称为$G_{1}$。构造Wigner cell $E_{1}$。因为我们以$G_{1}$构造$E_{1}$，所以$E_{1}$内的点不可能距离$G_{n},\ n\geq 2$最近。而显然$E_{1}$内的点距离原点比距离$G_{1}$更近，所以$E_{1}$就是FBZ。

![[Drawing 2026-04-14 18.58.39.excalidraw|center|500]]
为了构造$\text{BZ}_{2}$，我们选择与原点第二近的点$G_{2}$。同样构造Wigner cell $E_{2}$。因为我们以$G_{2}$构造$E_{2}$，所以$E_{2}$内的点不可能距离$G_{n},\ n\geq 3$最近。而显然$E_{2}$内的点也不可能距离$G_{2}$最近，所以只能距离$G_{1}$或者$0$最近。那么$E_{2} / E_{1}$距离$0$第二近，为$\text{BZ}_{2}$。
