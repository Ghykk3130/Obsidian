...
这是外场对偶极的力。将偶极搬运到外场中需要相反地力做功。于是偶极在外场中的能量为：$-\int \vec{F} \cdot d\vec{r}$ 。
...
3） $\begin{aligned} \vec{\tau} & =\int \vec{r}^{\prime} \times \rho d r \vec{E}\left(\vec{r}_0+\vec{r}^{\prime}\right) \\ & =\int \vec{r}^{\prime} \times \rho d r\left(\vec{E}\left(\vec{r}_0\right)+\vec{r}^{\prime} \times \nabla \vec{E}\left(\vec{r}_0\right)\right) \\ & =\vec{p} \times \vec{E}\end{aligned}$
