...
^definition41
>[!Theorem 3]
>Let $V$ be a set over which $g^t$ is ergodic. Let $f(q,p)\in L^1(V)$. Then:
>$$\bar{f}= \langle f\rangle= \frac{1}{m(V)}\int_{V}dm f\text{, for the time average starting points a.e. on }V$$
## Ex:
令$V_{1},V_{2}$为$V$的一个metric decomposition，满足[[统计力学的数学基础#^9f5cd3|definition 4.1]]。令$f(q,p):V\rightarrow \mathbb{R}, f=\mathbb{1}_{V_{1}}$
