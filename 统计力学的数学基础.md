...
>$$g^tV=V,\forall t$$
>If $\exists V$ measurable, and there exist measurable invariant sets $V_{1},V_{2}$ such that:
>$$V=V_{1}\cup V_{2}$$
>Then say that $g^t$ is metrically decomposable.

>[!Theorem 3]
>Let $V$ be a metrically indecomposable set, $f(q,p)$ be a function of microstates. Then:
>$$\bar{f}= \frac{1}{m(V)}\int_{V}dm f\text{, for the time average starting points a.e.}$$
