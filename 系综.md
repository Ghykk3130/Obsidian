...
After normalization:
$$\rho= \frac{\exp(-\beta E_{s}+\beta \mu N_{s})}{Q}$$
连续化：
$$\rho d^{3N}qd^{3N}p= \frac{d^{3N}qd^{3N}p}{h^{3N}N!} \frac{\exp(-\beta E+\beta \mu N)}{Q}$$
可以定义巨配分函数：
$$Q= \sum_{N} \int \frac{d^{3N}qd^{3N}p}{h^{3N}N!} \exp(-\beta E+\beta \mu N) $$
