...
将系数作为$v_{F}$，得到Hamiltonain $v_{F}(\hat{z}\times \vec{\sigma})\cdot \vec{k}_{\perp}$。在另一个表面，由于晶体场方向相反，得到Hamiltonian为$-v_{F}(\hat{z}\times \vec{\sigma})\cdot \vec{k}_{\perp}$。结合两种情况写出Hamiltonian就是：
$$\tau^zv_{F}(\hat{z}\times \vec{\sigma})\cdot \vec{k}_{\perp}c_{\vec{k}_{\perp}}^{\dagger}c_{\vec{k}_{\perp}}$$

若加上磁场，层内电子自旋还会和磁场耦合。得到Hamiltonian为$m\sigma^z$。
...
1 & 0
\end{pmatrix}$$
$\tau^x$能将上表面态，在不改变其他量子数的情况下flip到下表面。所以表示tuneling的Hamiltonian为$\Delta_{S}\tau^x$。

考虑不同层之间的tuneling。只存在两种：1）从某层上表面tuneling到更上一层的下表面。2）从某层下表面tuneling到更下一层的上表面。

前者Hamiltonian为$\frac{1}{2}\Delta_{D}\tau^{-}$


我们propose这样一个Hamiltonian：
...
对于任意有上面那些量子数确定的ket，Hamiltonian中每个算符通过张量的方式作用。
