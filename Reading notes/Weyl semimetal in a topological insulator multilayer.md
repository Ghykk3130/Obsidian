...
显然层数$i$可以作为一个量子数。Fix $i$之后，每层分为上下两个表面，作为一个量子数。每个表面的电子是2D的，故$\vec{k}_{\perp}=(k_{x},k_{y})$也是一个量子数。还有电子的自旋$\sigma$也是一个量子数。
...
$$H=\sum_{\vec{k}_{\perp},i,j}[v_{F}\tau^z(\hat{z}\times \vec{\sigma})\cdot \vec{k}_{\perp}\delta_{ij}]$$
