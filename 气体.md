...
我们现在来看$L^{2}$是否能直接用广义坐标和广义动量表示出来。选取球坐标系。
...
定义一维动量${p}_{r}=\mu  \dot{r}_{i}$。那么动能为：
$$\frac{P^{2}}{2M}+ \frac{p_{r}^{2}}{2\mu}+ \frac{L^{2}}{2\mu^{}r^{2}}$$
假设分子震动很小，假设震动很小，那么角动量项分母直接用平衡点位形替代：
...
下面来思考上式的自由度问题。首先，两个分子各自有三个自由度。所以总共有六个力学自由度。我们经过了一次换元，将$\vec{r}_{1},\vec{r}_{2}$换为$\vec{R},\vec{r}$。还是有六个力学自由度。其中，$\vec{R}$的conjugate momentum为$\vec{P}$。这是因为：
$$\begin{align}
...
Claim that $\frac{L^{2}}{2I}$中有两个力学自由度。
