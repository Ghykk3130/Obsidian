...
$$V(r)=V(r^{*})+ \frac{\partial V}{\partial r}(r-r^{*})+ \frac{1}{2} \frac{\partial^{2}V}{\partial r^{2}}(r-r^{*})^{2}=V(r^{*})+ \frac{1}{2} \frac{\partial^{2}V}{\partial r^{2}}(r-r^{*})^{2}$$
...
\end{align}$$
为了形成漂亮的conjugation关系 $\vec{p}_{r}$可以定义为$\mu \frac{d}{dt}(r -r^{*})$，不会影响任何东西。