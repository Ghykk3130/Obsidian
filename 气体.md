...
\end{align}$$
现在继续拆解$\frac{1}{2}\mu|\dot{\vec{r}}|^{2}$，想拆出一个旋转项。我们有：
$$\begin{align}
...
定义一维动量${p}_{r}=\mu e_{i}  \dot{r}_{i}$。那么动能为：
...
Claim that $\frac{L^{2}}{2I}$中有两个力学自由度。为此，先作如下命题：

>[!Note] Proposition 1
>$$\vec{L}=\mu r^{2}\hat{r}\times  \dot{\hat{r}}$$
## Proof.
由claim，我们有：
$$L^{2}=\mu^{2}r^{2}(2\vec{\omega}\cdot(\vec{r}\times(e_{i}  \dot{r_{i}}))+r^{2}\omega^{2}-(\vec{r}\cdot \vec{\omega})^{2})$$
>[!Note] Idea
