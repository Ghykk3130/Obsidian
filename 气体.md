...
$$\begin{align}
 & L= \frac{P^{2}}{2M}+ \frac{p_{r}^{2}}{2\mu}+ \frac{L^{2}}{2I}- \frac{1}{2}k(r-r^{*})^{2}= \frac{1}{2}M(  \dot{R_{i}})^{2}+ \frac{p_{r}^{2}}{2\mu }+ \frac{L^{2}}{2I}- \frac{1}{2}k(r-r^{*})^{2} \\
\implies & \text{momentum conjugate to }R_{i}= \frac{\partial L}{\partial  \dot{R_{i}}}=  M  \dot{R_{i}}=P_{i}
\end{align}$$
 $L^{2}$中有两个自由度。这是因为$\vec{L}= \overset{\leftrightarrow}{I}\cdot \vec{\omega}$。而$\vec{\omega}=$
